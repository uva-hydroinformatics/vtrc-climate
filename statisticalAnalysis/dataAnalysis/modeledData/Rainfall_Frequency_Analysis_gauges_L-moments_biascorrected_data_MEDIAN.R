...
PDSRain <- function(Years){
  ...
  threshold <- 80 # lower threshold for 1-year return period (mm) for the selected station
...
dirc <- "F:/Projects/Bridges Climate Change Project/Statistical Analysis/Climate DATA/Climate Models/RCMs_NA_Cordex_i/BiasCorrection/GCMs/RCP85/Projection/"
...
IDF_output <- data.frame(matrix(ncol = 10, nrow = 0))
...
for(model in unlist(model_list)){
  ...
  YearBaseline <- seq(from=2071, to=2100, by=1)
  ...
  selected_rainfall <- sort(unlist(Max_DailyRain_Baseline, use.name=FALSE), decreasing=FALSE) * 1.13 # Maybe this values need to be multiple by 1.13 as in the Maryland report
...
colnames(IDF_output) <- c("500yr", "100yr", "50yr", "25yr", "10yr", "5yr", "2yr", "1.25yr", "1.11yr")
IDF_output["Models"] <- unlist(model_list)

# write output to csv
write.csv(IDF_output, file=paste0(dirc, "24Hour_Rainfall_Historical_ML_estimation_Median_PDS80_2100.csv"))
