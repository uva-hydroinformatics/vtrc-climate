...
rainfall_data <- read.csv("./output/REPORT Maryland/USW00013773.csv", header = TRUE)


# include only the available start and end full year
YearBaseline <- seq(from=1946, to=2012, by=1)
...
Max_DailyRain_Baseline <- Max_DailyRain_Baseline[c(7:67)]
...
years <- c("500yr", "100yr", "50yr", "25yr", "10yr", "5yr", "2yr", "1yr")
output_lines <- c("24-hour Rainfall IDF generated from historical observation:", years, round(returns_Baseline,1))
output_IDF <- file("./output/REPORT Maryland/USW00013773_24H_IDF_CF.csv")
